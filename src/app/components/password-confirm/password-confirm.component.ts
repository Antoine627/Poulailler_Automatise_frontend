...
export class PasswordConfirmComponent implements OnInit {
  ...
  private processToken(): void {
    ...
    this.authService.confirmPasswordChange(this.token).subscribe({
      next: (response) => {
        ...
        this.message = response.message || 'Mot de passe modifié avec succès!';
        this.loading = false;
        this.startCountdown();
      },
      error: (error) => {
        this.handleError(error.error?.message || 'Erreur lors de la confirmation');
      }
    });
  }
  ...
  private startCountdown(): void {
    this.countdownTimer = setInterval(() => {
      ...
      if (this.countdown <= 0) {
        clearInterval(this.countdownTimer);
        // Si succès, déconnecte et redirige vers login
        if (this.success) {
          this.authService.logout();
          localStorage.setItem('passwordChangeSuccess', 'true');
        }
        this.router.navigate([this.success ? '/login' : '/settings']);
      }
    }, 1000);
  }
}